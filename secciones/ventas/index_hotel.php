...
if ($_POST) {
    ...
    if (isset($_POST["actualizar_id_tiempo"])) {
      ...
      if($lista_tiempos['estado_tiempo'] == 1){
        ...
        SET t.fecha_fin=CURRENT_TIMESTAMP(),
          ...
          t.precio_final=CEIL(TIME_TO_SEC(SEC_TO_TIME(TIMESTAMPDIFF(SECOND, t.fecha_inicio, CURRENT_TIMESTAMP()))) / 86400) * c.precio_cuenta
    ...
    if (isset($_POST["id_cuenta_end"])) {
      ...
      SET t.fecha_fin=CURRENT_TIMESTAMP(),
          t.tiempo_invertido=SEC_TO_TIME(TIMESTAMPDIFF(SECOND, t.fecha_inicio, CURRENT_TIMESTAMP())),
          t.precio_final=CEIL(TIME_TO_SEC(SEC_TO_TIME(TIMESTAMPDIFF(SECOND, t.fecha_inicio, CURRENT_TIMESTAMP()))) / 86400) * c.precio_cuenta, t.estado_tiempo=0,
    ...
    if (isset($_POST["id_cuenta_continuar"])) {
      ...
      SET t.fecha_fin=CURRENT_TIMESTAMP(),
          t.tiempo_invertido=SEC_TO_TIME(TIMESTAMPDIFF(SECOND, t.fecha_inicio, CURRENT_TIMESTAMP())),
          t.precio_final=CEIL(TIME_TO_SEC(SEC_TO_TIME(TIMESTAMPDIFF(SECOND, t.fecha_inicio, CURRENT_TIMESTAMP()))) / 86400) * c.precio_cuenta, t.estado_tiempo=1,
    ...
    if (isset($_POST["id_cuenta_liquidar"])) {
      ...
      foreach ($lista_productos_liquidados as $registro) {
        ...
        foreach ($lista_tiempos_liquidados as $registro) {
          $inicio = date("d/M", strtotime($registro['fecha_inicio']));
          $fin = date("d/M", strtotime($registro['fecha_fin']));
                    ...
                    <tr class="" valign="middle" align="center">
                        ...
                        <?php if (isset($registro_tiempo['estado_tiempo']) && $registro_tiempo['estado_tiempo'] == 1) {?>
                          <button type="submit" value=""
                            ...
                            class="btn btn-danger">Check-Out
                          </button>
                        <?php }?>
                    ...
                    <tr class="" valign="middle" align="center">
                        ...
                        <?php
                          if (isset($registro_tiempo['fecha_inicio']) && $registro_tiempo['estado_tiempo']==1) {
                            echo date('d-M-y H:i', strtotime($registro_tiempo['fecha_inicio']));
                        ...
                        <?php
                          if (isset($registro_tiempo['fecha_fin']) && $registro_tiempo['estado_tiempo']==1) {
                              echo date('d-M-y H:i', strtotime($registro_tiempo['fecha_fin']));
                        ...
                        <?php
                            ...
                            if (isset($registro_tiempo['tiempo_invertido']) && 
                                ($registro_tiempo['estado_tiempo'] == 1 || $registro_tiempo['estado_liquidado'] == 1)) {
                                $tiempo = $registro_tiempo['tiempo_invertido'];
                            }
                            ...
                            if ($tiempo && $tiempo !== "00:00:00") {
                                ...
                                // Calcular días (mínimo 1 si hay tiempo)
...
<script>
  ...
  function liquidar_sin_tiempo_detenido(id_cuenta_liquidar) {
    Swal.fire({
        title: 'Debes hacer Check-out antes de liquidar',
        icon: 'error',
        timer: 5000,
        timerProgressBar: true,
        didClose: () => {
